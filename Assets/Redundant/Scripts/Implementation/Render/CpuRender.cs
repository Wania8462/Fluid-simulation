...
public class CreateCubeMesh : MonoBehaviour
{
    ...
    [SerializeField] private CpuFluidSimulation fluidSimulation;
    ...
    void LateUpdate()
    {
        DrawPoints(fluidSimulation.points, fluidSimulation.particleSize);
    }
    ...
    public void DrawPoints(float3[] points, float3 scale)
    {
        if (matrices == null)
        {
            matrices = new Matrix4x4[fluidSimulation.numParticles];
        }

        for (int i = 0; i < fluidSimulation.numParticles; i++)
        {
        ...
        for (int i = 0; i < matrices.Length; i += batchSize)
        {
            Graphics.DrawMeshInstanced(
                ...
                0,
                mat,
                matrices.ToList().GetRange(i, Mathf.Min(batchSize, matrices.Length - i))
            );
        }
    }
